...
import { Frame, getPinFromPos, Pin, PinState, PinValue } from "./types.js";

export function calculateTotals(game: Game) {
  ...
  const penalties = deliveries.map(e => e.foul ? -15 : 0).reduce((a, b) => a + b, 0 as number);
...
export function calculatePerFrameTotals(game: Game): (number | null)[] {
  const simple = simplifyFrames(game.frames);
...
export function simplifyFrames(frames: Frame[]) {
  return frames.map((frame, index) => {
    ...
    if (type === "SPARE" && index !== 9) {
      ...
      return { deliveries: [frame.deliveries[0], frame.deliveries[1], { pins: nextBall, foul: false }] } as Frame
    }
    if (type === "STRIKE" && index !== 9) {
      ...
      return { deliveries: [frame.deliveries[0], { pins: nextBall, foul: false }, { pins: secondNextBall, foul: false }] } as Frame
    }
    return {
      deliveries: [
...
export function addPins(...pins: PinState[]): PinState {
  return [
    pins.some((arr) => arr[0]),
    pins.some((arr) => arr[1]),
    pins.some((arr) => arr[2]),
...
export function getPinCount(pins: PinState) {
  return pins.map((e, index) => e ? PinValue[getPinFromPos(index)] : 0).reduce((a, b) => a + b, 0)
}

export function getFrameType(frame: Frame): "OPEN" | "SPARE" | "STRIKE" | "INCOMPLETE" {
  const firstBall = frame.deliveries[0];
  if (!firstBall) return "INCOMPLETE";
  var rollingPins = firstBall.pins;
  if (rollingPins.every(Boolean)) return "STRIKE";
  const secondBall = frame.deliveries[1];
  if (!secondBall) return "INCOMPLETE";
  rollingPins = addPins(firstBall.pins, secondBall.pins);
  if (rollingPins.every(Boolean)) return "SPARE";
  const thirdBall = frame.deliveries[2];
  if (!thirdBall) return "INCOMPLETE";
  return "OPEN"
}